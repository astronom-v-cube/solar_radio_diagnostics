...
# реализация гененирования чисто int чисел не доделана!!!!!!!!!!!! КОСТЫЛЬ
    ...
    def generate(self, points, method, remove_lag = 0):

        try:
            if remove_lag > 0:
                if self.gen > remove_lag:
                    for a in ['x', 'y', 'r']:
                        os.remove(f'{self.fname}_gen_{self.gen - remove_lag - 1}_{a}.txt')
        except: pass
    ...
    def Generating(self, ngenerations, nchildren, sigmacoeff, points, method, do_plot = False, continuation = False, continuation_gen = None, refx = None, remove_lag = 0):
